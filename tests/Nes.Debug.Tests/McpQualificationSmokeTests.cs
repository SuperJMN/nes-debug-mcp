...
{
    [Theory]
    [InlineData(QualificationBackend.AprNes)]
    [InlineData(QualificationBackend.Adnes)]
    public async Task Generated_nrom_passes_the_real_stdio_smoke(QualificationBackend backend)
    {
    ...
    {
        using var rom = TemporaryTestFile.FromBytes(CreateHighBankMmc3());
        await using var client = McpStdioClient.Start(FindServerAssembly(), QualificationBackend.AprNes);
    ...
    private static byte[] CreateHighBankMmc3()
    {
        const int headerSize = 16;
        const int prgSize = 2 * 16 * 1024;
