...
The expected total and every per-mapper count are mandatory, positive, and consistent: the mapper counts must sum to `--expected-total`. A missing or unexpected cohort, a qualification failure, a timeout, or a worker/protocol failure produces a nonzero exit code.
